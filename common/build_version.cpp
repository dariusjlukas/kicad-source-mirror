...
{
    // KICAD_BUILD_TIMESTAMP is regenerated by the version_header CMake step
    // on every ninja invocation, so this reflects the actual link time of
    // the current binary rather than the last time build_version.cpp itself
    // happened to recompile (which is what __DATE__/__TIME__ would give).
    return wxString::FromUTF8( KICAD_BUILD_TIMESTAMP );
}
